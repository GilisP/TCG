...
namespace TCG.Table
{
 public sealed class NetworkSession:MonoBehaviour
 {
  ...
  public static string ContentHash(){using(var sha=SHA256.Create()){string sources=string.Join("\n",Directory.GetFiles(Path.Combine(Application.streamingAssetsPath,"Expansions"),"*.json").OrderBy(x=>x,StringComparer.Ordinal).Select(File.ReadAllText));return BitConverter.ToString(sha.ComputeHash(Encoding.UTF8.GetBytes("tcg-network-1/20260924\n"+sources))).Replace("-","");}}
  ...
  public async Task Host(bool internet,bool isPublic,string name,int count,bool teams,string nick,ushort listenPort=7777)
  {
   ...
   try{if(internet)await Services();room=new NetworkRoom(catalog,effects,fingerprint,count,teams,()=>Time.realtimeSinceStartupAsDouble);CreateManager();if(internet){session=await MultiplayerService.Instance.CreateSessionAsync(new SessionOptions{Name=name,MaxPlayers=count,IsPrivate=!isPublic,SessionProperties=new Dictionary<string,SessionProperty>{{"tcg",new SessionProperty(fingerprint,VisibilityPropertyOptions.Public,PropertyIndex.String1)},{"mode",new SessionProperty(teams?"teams":"ffa")}}}.WithRelayNetwork());}else{transport.SetConnectionData("127.0.0.1",port,"0.0.0.0");if(!manager.StartHost())throw new InvalidOperationException("Não foi possível abrir a porta da sala.");}Status="Sala criada · "+Code;}catch{await Stop();throw;}finally{Busy=false;}
  }
  ...
  async Task Services(){if(string.IsNullOrEmpty(Application.cloudProjectId))throw new InvalidOperationException("Vincule este projeto em Unity > Project Settings > Services para habilitar salas pela internet.");if(UnityServices.State!=ServicesInitializationState.Initialized)await UnityServices.InitializeAsync();if(!AuthenticationService.Instance.IsSignedIn)await AuthenticationService.Instance.SignInAnonymouslyAsync();}
  ...
  public async Task Reconnect(){if(!reconnecting||Time.unscaledTime>=reconnectUntil)throw new InvalidOperationException("Prazo de reconexão expirado.");Busy=true;try{if(cloud&&session!=null){manager.Shutdown();await Task.Delay(500);session=await MultiplayerService.Instance.ReconnectToSessionAsync(session.Id,new ReconnectSessionOptions{Type=session.Type});}else{manager.Shutdown();await Task.Delay(500);transport.SetConnectionData(address,port);manager.StartClient();}}finally{Busy=false;}}
