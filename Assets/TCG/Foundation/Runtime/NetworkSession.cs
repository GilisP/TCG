...
namespace TCG.Table
{
 public sealed class NetworkSession:MonoBehaviour
 {
  ...
  public static string ContentHash(){using(var sha=SHA256.Create()){string sources=string.Join("\n",Directory.GetFiles(Path.Combine(Application.streamingAssetsPath,"Expansions"),"*.json").OrderBy(x=>x,StringComparer.Ordinal).Select(File.ReadAllText));return BitConverter.ToString(sha.ComputeHash(Encoding.UTF8.GetBytes("tcg-network-3/20260924-relay-progress\n"+sources))).Replace("-","");}}
